...
function handleClick(clickedButtonEvent) {
    ...
    let txt = card.button.innerText;

    if (txt == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (txt == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    card.productName.insertAdjacentHTML('afterend', `<div class="desc">${txt1}</div>`);
